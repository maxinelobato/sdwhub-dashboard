...
const DESAFIO_COLORS= ['#f43f5e', '#7c5cbf', '#f59e0b', '#e2d5b0', '#10b981', '#60a5fa'];
const CLAREZA_COLORS= ['#f43f5e','#f43f5e','#f59e0b','#f59e0b','#e2d5b0','#10b981'];
...
export const PCEDashboard = ({ data, participants }: Props) => {
  ...
  const solosCount   = data.equipe.find((e) => e.label.toLowerCase().includes('sozinho'))?.count ?? 0;
      ...
      <div className="relative z-10">
        ...
        <header className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div className="flex items-center gap-4">
            <Image src="/images/sdw-logo-gold.png" alt="PCE" width={56} height={56} priority className="h-12 w-auto" />
            <div>
        ...
        <div
          role="tablist"
          className="mb-6 flex gap-1 rounded-full border border-white/10 bg-white/5 p-1 w-fit"
        >
          {TABS.map((t) => {
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="mb-5 grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <KpiCard
                  ...
                  icon={<LightbulbIcon size={18} weight="bold" />}
                ...
                <KpiCard
                  ...
                  icon={<UserFocusIcon size={18} weight="bold" />}
                  accent="bg-rose-500"
                  progress={dependPct}
                  delay={0.15}
                />
              ...
              <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Fase da Empresa" subtitle={`${total} respondentes`} delay={0.2}>
                  <DonutChart data={data.faseEmpresa} total={total} colors={FASE_COLORS} />
                </Panel>
                <Panel title="Faturamento Mensal" subtitle="distribuição por faixa de receita" delay={0.25}>
                  <ReBarChart data={data.faturamento} total={total} color="#e2d5b0" labelTruncate={22} />
                </Panel>
              </div>

              {/* Row 2: Clareza (column) + Equipe (donut) */}
              <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Nível de Clareza (0 a 5)" subtitle="sobre como crescer sua empresa" delay={0.3}>
                  <ReColumnChart data={data.clareza} colors={CLAREZA_COLORS} height={200} />
                </Panel>
                <Panel title="Tamanho da Equipe" subtitle="pessoas na operação" delay={0.35}>
                  <DonutChart data={data.equipe} total={total} colors={EQUIPE_COLORS} />
                </Panel>
              </div>

              {/* Row 3: Investimento (h-bar) + Satisfação (column) */}
              <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Investimento em Crescimento" subtitle="frequência de investimento no negócio" delay={0.4}>
                  <ReBarChart data={data.investimento} total={total} color="#f59e0b" labelTruncate={28} />
                </Panel>
                <Panel title="Satisfação com Ambiência" subtitle="escala 1–5 · ambiente / mesa atual" delay={0.45}>
                  <ReColumnChart
                    data={data.satisfacaoAmbiente}
                    colors={['#f43f5e','#f59e0b','#f59e0b','#e2d5b0','#10b981']}
                    height={200}
                  />
                </Panel>
              </div>
              ...
              <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Cidades" subtitle="top cidades · onde moram os respondentes" delay={0.5}>
                  <ReBarChart data={data.cidades} total={total} color="#7c5cbf" labelTruncate={20} />
                </Panel>
                <Panel title="Faixa Etária" subtitle="distribuição por idade" delay={0.55}>
                  <ReBarChart data={data.faixaEtaria} total={total} color="#2dd4bf" labelTruncate={22} />
                </Panel>
              </div>
            </motion.div>
          )}
          ...
          {tab === 'desafios' && (
            ...
            >
              ...
              <div className="mb-4">
                <Panel title="Maiores Desafios (Top Menções)" subtitle="múltipla escolha · até 3 por resposta" delay={0.2}>
                  <ReColumnChart
              ...
              <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Canais de Venda" subtitle="múltipla escolha · como acontecem as vendas" delay={0.3}>
                  <ReBarChart data={data.canaisVenda} total={total} color="#10b981" labelTruncate={20} />
                </Panel>
                <Panel title="Dependência do Dono" subtitle="negócio depende de você para vender?" delay={0.35}>
                  <DonutChart data={data.dependencia} total={total} colors={DEP_COLORS} />
                </Panel>
              </div>

              {/* Segmentos */}
              <div className="mb-4">
                <Panel title="Segmentos / Área de Atuação" subtitle="top 12 · principais mercados representados" delay={0.4}>
                  <ReBarChart data={data.segmentos} total={total} color="#60a5fa" maxItems={12} labelTruncate={36} />
                </Panel>
              </div>
            </motion.div>
          )}
        ...
        <footer className="mt-6 flex flex-col items-center justify-between gap-2 text-[10px] font-black tracking-[0.3em] text-white/30 uppercase sm:flex-row">
          ...
          <span>Período: <span className="text-brand-cream">06 a 14 abr 2026</span></span>
        </footer>
      </div>
    </section>
  );
